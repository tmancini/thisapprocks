...
public class ImageContainer extends Container {
  ...
  public final static int MAX_IMAGES = 1;
  ...
  public ImageContainer() {
    ...
    cameraFab.addActionListener(event -> {
      if(imageViewer.getImageList() == null || imageViewer.getImageList().getSize() < MAX_IMAGES) {
    ...
    add(BorderLayout.CENTER, LayeredLayout.encloseIn(fab.bindFabToContainer(imageContainer), FlowLayout.encloseRight(removeButton)));
  }
    ...
    try {
      ...
      if(m == null) {
        ...
        m.addSelectionListener(
            (oldSelected, newSelected) -> statusLabel.setText(newSelected + 1 + " of " + imageViewer.getImageList().getSize()));
      }
      else {
      ...
      String path = filePath.substring(0, filePath.lastIndexOf("."))
          + imageViewer.getImageList().getSize() + filePath.substring(filePath.lastIndexOf("."));
  ...
  public String getImage() {
    return hasImage ? Base64.encode(EncodedImage.createFromImage(imageViewer.getImage(), true).getImageData()) : null;
  }
